...
@Component({
  selector: 'app-factura-minera-paso-dos',
  templateUrl: './factura-minera-paso-dos.component.html',
  styleUrls: ['./factura-minera-paso-dos.component.scss']
})
export class FacturaMineraPasoDosComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.parametricasSfeService.getTipoMetodoPago().subscribe((data) => {
            ...
            if (this.esFacturaVentaMineral()) this.listaTipoPago = this.listaTipoPago.filter(x=>!x.descripcion.toUpperCase().includes('GIFT'));
        });
        ...
        this.itemForm = this.fb.group({
            ...
            codigoMoneda: [this.item?.factura?.cabecera?.codigoMoneda],
            tipoCambio: [this.item?.factura?.cabecera?.tipoCambio],
            numeroTarjeta: [{ value: this.item?.factura?.cabecera?.numeroTarjeta ?? '', disabled: !this.item?.factura?.cabecera?.numeroTarjeta }],
            cafc:[''],
            fechaEmision:[''],
            numeroFactura:[''],
            idVenta:[null],
            direccionComprador: [this.item?.factura?.cabecera?.datosEspecificos?.direccionComprador, Validators.required],
        });
    }
    ...
    public async onSubmit():Promise<void>{
        if (this.backClicked) {
            this.router.navigate(['/adm/factura-minera-por-pasos/factura-minera-paso-uno']);
        } else {
   ...
   adicionarNuevoCliente() {
        ...
        ref.onClose.subscribe((res) => {
            if (res) {
                ...
                this.itemForm.patchValue({ direccionComprador: res.direccion });
            }
        });
    }
            ...
            next: (res) => {
                ...
                ref.onClose.subscribe((res2) => {
                    if (res2) {
                        ...
                        this.itemForm.patchValue({ direccionComprador: res2.direccion });
                    }
                });
            },
            error: (err) => {
    ...
    seleccionarCliente(event: any) {
        ...
        this.itemForm.patchValue({ direccionComprador: event?.direccion });
    }
    ...
    limpiarCliente() {
        ...
        this.itemForm.patchValue({ direccionComprador: '' });
        this.elmC?.focusInput();
    }
    ...
    getFacturaRecepcion(): FacturaRecepcion{
        let datosEspecificosTemp = this.item?.factura?.cabecera.datosEspecificos;
        datosEspecificosTemp!.direccionComprador = this.itemForm.controls['direccionComprador'].value;

        const cabecera : Cabecera = {
            datosEspecificos: datosEspecificosTemp,
