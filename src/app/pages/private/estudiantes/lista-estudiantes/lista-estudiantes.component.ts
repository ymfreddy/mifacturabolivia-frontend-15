...
export class ListaEstudiantesComponent implements OnInit {
    ...
    loadData(): void {
        this.loading = true;
        this.estudiantesService.getPaged(this.busqueda).subscribe({
            next: (res) => {
    ...
    deleteItem(item: Estudiante) {
        this.confirmationService.confirm({
            message: 'Esta seguro de eliminar a '+item.nombres+ ' ' +item.apellidoPaterno+ ' ' +item.apellidoMaterno +' ?',
            header: 'Confirmación',
            icon: 'pi pi-exclamation-triangle',
            accept: () => {
                this.estudiantesService.delete(item).subscribe({
                    next: (res) => {
        ...
        if (event.files.length) {
            ...
            reader.onload = (event: any) => {
                ...
                if (sheets.length) {
                    ...
                    this.estudiantesService.addMasive(rows).subscribe({
                        next: (res) => {
                            this.loadData();
                            this.informationService.showSuccess(res.message);
