...
@Component({
  selector: 'app-factura-ice-paso-dos',
  templateUrl: './factura-ice-paso-dos.component.html',
  styleUrls: ['./factura-ice-paso-dos.component.scss']
})
export class FacturaIcePasoDosComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.parametricasSfeService.getTipoMetodoPago().subscribe((data) => {
            ...
            // si es factura de ice se elimina todos los gifcard
            this.listaTipoPago = this.listaTipoPago.filter(x=>!x.descripcion.toUpperCase().includes('GIFT'));
        });
        ...
        this.itemForm = this.fb.group({
            ...
            montoIcePorcentual: [this.item?.factura?.cabecera?.montoIcePorcentual],
            montoIceEspecifico: [this.item?.factura?.cabecera?.montoIceEspecifico],
    ...
    public async onSubmit():Promise<void>{
        if (this.backClicked) {
            this.router.navigate(['/adm/factura-ice-por-pasos/factura-ice-paso-uno']);
        } else {
   ...
   adicionarNuevoCliente() {
    ...
    getFacturaRecepcion(): FacturaRecepcion{
        const cabecera : Cabecera = {
            ...
            montoTotalSujetoIva: this.itemForm.controls['montoTotalSujetoIva'].value,
